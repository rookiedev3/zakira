                    ...
                    <div class="md:col-span-2 grid md:grid-cols-[100px_1fr] gap-4 items-start">
                        <div>
                            ...
                            <div id="previewIcon"
                                 ...
                                 style="background-color: {{ old('platform') ? ($platforms[old('platform')]['color'] ?? '#999') : '#999' }}">
                                <i id="previewIconTag" class="{{ old('platform') ? ($platforms[old('platform')]['icon'] ?? 'fas fa-link') : 'fas fa-link' }}"></i>
                            </div>
                        </div>
                        <div>
                            ...
                            <select name="platform" id="platformSelect" required
                                ...
                                @foreach ($platforms as $name => $meta)
                                    <option value="{{ $name }}"
                                            ...
                                            @selected(old('platform') === $name)>
                                        {{ $name }}
                                    </option>
                                @endforeach
                                <option value="Lainnya (Kustom)" @selected(old('platform') === 'Lainnya (Kustom)')>Lainnya (Kustom)</option>
                            </select>
                            <p class="text-xs text-gray-400 mt-1">Logo dan warna brand otomatis terisi sesuai pilihan.</p>
                        </div>
                    </div>

                    {{-- Input kustom, muncul kalau pilih "Lainnya (Kustom)" --}}
                    <div id="customFields" class="md:col-span-2 grid md:grid-cols-2 gap-4 {{ old('platform') === 'Lainnya (Kustom)' ? '' : 'hidden' }}">
                        <div>
                            <label class="text-sm font-medium">Icon Class (Font Awesome)</label>
                            <input type="text" name="custom_icon_class" value="{{ old('custom_icon_class') }}" placeholder="fab fa-nama-platform"
                                   class="w-full border rounded-lg px-4 py-2 mt-1">
                        </div>
                        <div>
                            <label class="text-sm font-medium">Warna (hex)</label>
                            <input type="text" name="custom_color" value="{{ old('custom_color') }}" placeholder="#000000"
                                   class="w-full border rounded-lg px-4 py-2 mt-1">
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 mb-6">
                ...
                <form method="POST" action="{{ route('social-media.update', $editingSocialMedia) }}" class="grid md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2 grid md:grid-cols-[100px_1fr] gap-4 items-start">
                        ...
                        <div>
                            <label class="text-sm font-medium">Pilih Media Sosial</label>
                            <select name="platform" id="platformSelectEdit" required
                                    class="w-full border rounded-lg px-4 py-2 mt-1">
                                <option value="">-- Pilih platform --</option>
                                @foreach ($platforms as $name => $meta)
                                    <option value="{{ $name }}"
                                            data-icon="{{ $meta['icon'] }}"
                                            data-color="{{ $meta['color'] }}"
                                            @selected($editingSocialMedia->platform === $name)>
                                        {{ $name }}
                                    </option>
                                @endforeach
                                <option value="Lainnya (Kustom)" @selected($editingSocialMedia->platform === 'Lainnya (Kustom)')>Lainnya (Kustom)</option>
                            </select>
                            <p class="text-xs text-gray-400 mt-1">Logo dan warna brand otomatis terisi sesuai pilihan.</p>
                        </div>
                    </div>

                    <div id="customFieldsEdit" class="md:col-span-2 grid md:grid-cols-2 gap-4 {{ $editingSocialMedia->platform === 'Lainnya (Kustom)' ? '' : 'hidden' }}">
                        <div>
                            <label class="text-sm font-medium">Icon Class (Font Awesome)</label>
                            <input type="text" name="custom_icon_class" value="{{ $editingSocialMedia->platform === 'Lainnya (Kustom)' ? $editingSocialMedia->icon_class : '' }}" placeholder="fab fa-nama-platform"
                                   class="w-full border rounded-lg px-4 py-2 mt-1">
                        </div>
                        <div>
                            <label class="text-sm font-medium">Warna (hex)</label>
                            <input type="text" name="custom_color" value="{{ $editingSocialMedia->platform === 'Lainnya (Kustom)' ? $editingSocialMedia->color : '' }}" placeholder="#000000"
                                   class="w-full border rounded-lg px-4 py-2 mt-1">
                        </div>
                    </div>
